...
def dashboard_pie(request):
    if request.user.is_authenticated:
        open=TaskDetail.objects.filter(TASK_STATUS='Open').count()
        closed=TaskDetail.objects.filter(TASK_STATUS='Closed').count()
        reopen=TaskDetail.objects.filter(TASK_STATUS='Reopen').count()
        inprocess=TaskDetail.objects.filter(TASK_STATUS='Inprocess').count()
        expired=TaskDetail.objects.filter(TASK_STATUS='Expired').count()
        resolved=TaskDetail.objects.filter(TASK_STATUS='Resolved').count() 
        status_task = {
            'Open':open,
            'Closed':closed,
            'Reopen':reopen,
...
def Bar_chart(request):
    if request.user.is_authenticated:
        Task_Status=TaskDetail.objects.values_list('TASK_STATUS',flat=True)
        data={
